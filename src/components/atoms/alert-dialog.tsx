...
function AlertDialogOverlay({
  ...
  return (
    // pointer-events-auto: vaul's Drawer sets `pointer-events: none` on <body> while
    // open, which this dialog would otherwise inherit if rendered nested inside one.
    <ModalOverlayPrimitive
      data-slot="alert-dialog-overlay"
      className={cn(
        "fixed inset-0 isolate z-50 bg-black/10 pointer-events-auto duration-100 data-entering:animate-in data-entering:fade-in-0 data-exiting:animate-out data-exiting:fade-out-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    >
